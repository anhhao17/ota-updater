...
Result ManifestLoader::LoadFromFirstBundleEntry(OtaTarBundleReader& bundle, Manifest& out_manifest) {
    bool eof = false;
    BundleEntryInfo entry{};

    auto next_result = bundle.Next(entry, eof);
    if (!next_result.is_ok()) return next_result;
    ...
    if (entry_name != "manifest.json") {
        return Result::Fail(-1, "manifest.json must be the first entry in ota.tar (got: " + entry_name + ")");
    }

    std::string manifest_json;
    auto read_result = bundle.ReadCurrentToString(manifest_json);
    if (!read_result.is_ok()) return read_result;

    auto parsed = ManifestHandler::Parse(manifest_json);
    if (!parsed) return Result::Fail(-1, "Manifest parse error: " + parsed.error());
...
std::uint64_t BundlePreScanner::ComputeOverallTotalFromFile(const std::string& input_path,
    ...
    if (!bundle_result.is_ok()) {
        LogWarn("Pre-scan bundle open failed, overall progress disabled: %s", bundle_result.message().c_str());
        return 0;
    }
    ...
    if (!manifest_next.is_ok() || eof) return 0;
    (void)bundle.SkipCurrent();

    while (true) {
        auto next_result = bundle.Next(entry, eof);
        if (!next_result.is_ok()) return 0;
        if (eof) break;
