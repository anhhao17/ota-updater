...
ArchiveInstaller::ArchiveInstaller() : opt_() { opt_.mount_flags = MS_RELATIME; }

ArchiveInstaller::ArchiveInstaller(Options opt) : opt_(std::move(opt)) {
    if (opt_.mount_flags == 0)
...
Result ArchiveInstaller::InstallTarStreamToTarget(IReader& tar_stream,
                                                  ...
                                                  std::string_view tag) {
    if (install_to.empty())
        return Result::Fail(-1, "install_to is empty");
    ...
    auto drain_stream = [&]() -> Result {
        ...
        while (true) {
            const ssize_t n =
                tar_stream.Read(std::span<std::uint8_t>(drain_buf.data(), drain_buf.size()));
            if (n == 0)
                break;
            if (n < 0)
                return Result::Fail(-1, "archive stream drain failed");
        }
        return Result::Ok();
    };
