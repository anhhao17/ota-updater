...
void ConsoleProgressSink::OnProgress(const ProgressEvent& e) {
    ...
    if (e.overall_total > 0) {
        std::fprintf(stderr, "\r[%.*s] %3d%% | OTA %3d%%",
                     (int)e.component.size(), e.component.data(), comp_pct, ota_pct);
    } else {
        std::fprintf(stderr, "\r[%.*s] %3d%% | OTA --",
                     (int)e.component.size(), e.component.data(), comp_pct);
    }
    std::fflush(stderr);
