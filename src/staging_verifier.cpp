...
Result WriteAllToFd(int fd, std::span<const std::uint8_t> data) {
    ...
    while (off < data.size()) {
        ...
        if (n < 0) return Result::Fail(errno, "write failed");
...
Result TempFile::Create(TempFile& out) {
    ...
    if (fd < 0) return Result::Fail(errno, "mkstemp failed");
    out.fd_.Reset(fd);
    out.path_ = tmpl;
    return Result::Ok();
}
...
Result OtaEntryStager::StageAndVerify(std::unique_ptr<IReader>& entry_reader,
    ...
    if (expected_sha256.empty()) return Result::Fail(-1, "expected sha256 is empty");
    ...
    while (true) {
        ...
        if (n < 0) return Result::Fail(-1, "read failed while staging");
        if (n == 0) break;
    ...
    if (actual.empty()) return Result::Fail(-1, "sha256 compute failed");
    if (NormalizeHex(actual) != NormalizeHex(expected_sha256)) {
        return Result::Fail(-1, "sha256 mismatch: expected=" + expected_sha256 + " actual=" + actual);
    }

    auto file_reader = std::make_unique<FileOrStdinReader>();
    auto orr = FileOrStdinReader::Open(tmp.Path(), *file_reader);
    if (!orr.ok) return Result::Fail(-1, orr.msg);
